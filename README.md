...
**Benchmark evidence is being rebuilt.** The table previously published here
was generated by an earlier version of the harness that derived the
precision@k / recall@k / F1 review budget from the labels being scored
(`k = round(n_total * y.mean())`). That makes `k` equal the true number of
anomalies, which collapses all three metrics into one number by
construction — every row in the old table showed identical
`precision_at_k` / `recall_at_k` / `f1_at_contamination` values, a leak of
the answer into the operating point, not three independent signals. Rather
than leave numbers up that read as more informative than they are, the
table has been removed pending a fresh run of the fixed harness.

The harness (`src/sorethumb/evaluate/benchmark.py`) is now fixed: `k` comes
from a fixed, label-independent 5% review budget shared across every
dataset; ROC-AUC/AP are `n/a` (not a fabricated 0.0) when a sample is
single-class; and every run records its own generation timestamp, platform,
and Python/numpy/scipy/scikit-learn/sorethumb versions so a published number
can be tied to the environment that produced it.

Average precision (AP) is the headline metric once the table is
regenerated — it accounts for class imbalance in a way ROC-AUC does not.
...
This runs the full dataset × detector matrix, writes
`benchmark_results.md` / `.csv` / `benchmark_metadata.json`, and injects a
fresh table below (real datasets such as KDDCup99 and Covtype are fetched
and cached on first use, and the `one_class_svm` cells on the larger
datasets take a long time to fit — expect this to take a while).
